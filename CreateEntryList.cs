...
WITH 種目一覧 AS (
 ...
 --       AND プログラム.クラス番号 = エントリー.新記録判定クラス
...
FROM 
    選手
LEFT JOIN (SELECT * FROM 種目一覧 WHERE 種目番号 = 1) S1 ON 選手.選手番号 = S1.選手番号
LEFT JOIN (SELECT * FROM 種目一覧 WHERE 種目番号 = 2) S2 ON 選手.選手番号 = S2.選手番号
LEFT JOIN (SELECT * FROM 種目一覧 WHERE 種目番号 = 3) S3 ON 選手.選手番号 = S3.選手番号
